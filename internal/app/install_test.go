...
// 같은 명시적 경로를 우선하므로, APPDATA·LOCALAPPDATA는 임시 홈 하위 경로로 명시하고
// 나머지는 비웁니다. 그렇지 않으면 해당 변수가 설정된 CI 러너에서 모든 테스트가 실제
// 홈·캐시 디렉터리를 공유해 실행 순서에 따라 서로 간섭합니다.
// (LOCALAPPDATA를 비우면 Go 도구체인이 GOCACHE를 찾지 못하므로 비우지 않습니다.)
