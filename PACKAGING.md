...
> **참고**: GitHub에서 제공하는 자동 소스 코드 tarball(`tarball_url`)은 최상위에 `owner-repo-[hash]` 형태의 디렉토리를 포함합니다. `ppm`은 압축 해제된 디렉토리를 최대 3단계 깊이까지 자동으로 검색하여 알맞은 바이너리를 매칭하므로, 디렉토리 구조가 한 단계 더 감싸져 있어도 정상적으로 동작합니다.
...
### 바이너리 탐색 우선순위
아카이브 내부에서 실행 파일을 식별할 때 `ppm`은 다음 순서로 적합한 바이너리를 선택합니다:
1. 파일명이 `bin_name`(또는 레포지토리명)과 **정확히 일치**하는 파일
2. 파일명에 `bin_name`이 **포함**된 파일 (대소문자 무시)
3. 디렉토리 내에 실행 파일(Unix의 `x` 권한 보유 파일, Windows의 `.exe` 파일)이 **단 하나만** 있는 경우 그 파일 선택
...
예시 파일은 이 레포지토리의 [ppm.json.example](ppm.json.example)을 참고하세요.
